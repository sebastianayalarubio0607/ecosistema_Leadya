...
class MetaPageStatusHistoryController extends Controller
{
    public function index(Request $request): View
    {
        $items = MetaPageStatusHistory::query()
            ->with(['customer:id,name', 'page:id,meta_page_id,name', 'webhookEvent:id,object,field'])
        ...
        return view('meta.pages.status_history', [
            'items' => $items,
            'customers' => Customer::query()->orderBy('name')->get(['id', 'name']),
            'pages' => MetaPage::query()->orderBy('name')->get(['id', 'meta_page_id', 'name']),
            'statuses' => MetaPageStatusHistory::query()
                ->select(['estado_meta_nuevo', 'estado_meta_nuevo_nombre'])
                ->whereNotNull('estado_meta_nuevo')
                ->distinct()
                ->orderBy('estado_meta_nuevo')
                ->get(),
            'queryTypes' => MetaPageStatusHistory::query()
