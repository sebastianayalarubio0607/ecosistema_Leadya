    ...
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                <a href="{{ route('meta.ad-sets.index') }}" class="w-full text-center px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Limpiar</a>
            </div>
        </form>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-white/5 text-white/70">
                    <tr>
                        ...
                        <th class="text-left px-3 py-2">Optimization</th>
                ...
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($items as $it)
                        <tr class="hover:bg-white/5">
                            ...
                            <td class="px-3 py-2">
                                {{ $it->name }}
                                <div class="text-xs text-white/50">{{ $it->meta_ad_set_id }}</div>
                            </td>
                            <td class="px-3 py-2">{{ $it->optimization_goal }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-white/60">No hay ad sets.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
