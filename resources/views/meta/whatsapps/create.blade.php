...
@section('title', 'Nueva Meta WhatsApp')
...
@section('content')
    @php($whatsapp = $whatsapp ?? new \App\Models\MetaWhatsapp(['status' => true]))

    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="POST" action="{{ route('meta.whatsapps.store') }}" class="space-y-4">
            @csrf

            @include('meta.whatsapps._form')
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('meta.whatsapps.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
