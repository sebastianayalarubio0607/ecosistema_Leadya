...
@section('title', 'Historial estados Meta Pages')
...
@section('header_actions')
    ...
    <a href="{{ route('meta.pages.index') }}" class="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white border border-white/10">Paginas</a>
@endsection
...
@section('content')
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div class="md:col-span-3">
                <label class="block mb-1 text-white/70">Pagina de Meta</label>
                <select name="meta_page_id" class="w-full rounded-xl border border-white/10 p-2 bg-slate-900/60 text-white">
                    <option value="">Todas</option>
                    @foreach($pages as $page)
                        <option value="{{ $page->id }}" @selected((string) request('meta_page_id') === (string) $page->id)>{{ $page->name ?: $page->meta_page_id }} ({{ $page->meta_page_id }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-12 flex flex-wrap gap-2">
                ...
                <a href="{{ route('meta.pages.status-history.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Limpiar</a>
            </div>
        </form>
            ...
            <table class="w-full min-w-[1300px] text-sm" data-sortable-table>
                <thead class="bg-white/5 text-white/70">
                    <tr>
                        ...
                        <x-sort-header :index="2" label="Meta Page ID" />
                ...
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($items as $item)
                        <tr class="hover:bg-white/5">
                            ...
                            <td class="px-3 py-2">
                                <div class="font-semibold">{{ $item->meta_page_external_id ?: $item->page?->meta_page_id }}</div>
                                <div class="text-xs text-white/50">{{ $item->page?->name }}</div>
                            </td>
