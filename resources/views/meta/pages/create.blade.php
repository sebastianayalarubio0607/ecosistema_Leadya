...
@section('title', 'Crear Meta Page')
@section('subtitle', 'Registrar página Meta en el CRM')
...
@section('content')
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="POST" action="{{ route('meta.pages.store') }}" class="space-y-4">
            @csrf
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('meta.pages.index') }}" class="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white border border-white/10">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
