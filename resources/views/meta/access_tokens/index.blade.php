...
@section('subtitle', 'Tokens generales y credenciales WhatsApp por usuario del sistema')
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            <div class="md:col-span-3">
                ...
                <input name="search" value="{{ request('search') }}"
                       ...
                       placeholder="Nombre, tipo, app id o error">
            </div>

            <div class="md:col-span-3">
                <label class="block mb-1 text-white/70">Proposito</label>
                <select name="purpose" class="w-full rounded-xl border border-white/10 p-2 bg-slate-900/60 text-white">
                    <option value="">-- Todos --</option>
                    @foreach($purposes as $purpose)
                        <option value="{{ $purpose }}" @selected(request('purpose') === $purpose)>
                            {{ $purpose }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="w-full max-w-full overflow-x-auto rounded-xl border border-white/10 [scrollbar-gutter:stable]" data-sortable-table-wrap>
            ...
            <table class="w-full min-w-[1300px] text-sm" data-sortable-table>
                <thead class="bg-white/5 text-white/70">
                    <tr>
                        <x-sort-header :index="0" label="Nombre" />
                        <x-sort-header :index="1" label="Proposito" />
                        <x-sort-header :index="2" label="Tipo" />
                        <x-sort-header :index="3" label="Customer" />
                        <x-sort-header :index="4" label="Meta App ID" />
                        <x-sort-header :index="5" label="Default" />
                        <x-sort-header :index="6" label="Activo" />
                        <x-sort-header :index="7" label="Ultima validacion" />
                        <x-sort-header :index="8" label="Acciones" class="w-72" />
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($items as $item)
                        <tr class="hover:bg-white/5">
                            ...
                            <td class="px-3 py-2">{{ optional($item->last_validated_at ?? $item->refresh_last_run_at)->format('Y-m-d H:i') ?: '—' }}</td>
                            <td class="px-3 py-2">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    @if(($item->purpose ?: \App\Models\MetaAccessToken::PURPOSE_GENERAL) !== \App\Models\MetaAccessToken::PURPOSE_WHATSAPP)
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-3 py-8 text-center text-white/60">No hay tokens Meta registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
