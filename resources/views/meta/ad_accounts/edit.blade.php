...
@section('title', 'Editar Meta Ad Account')
...
@section('content')
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="POST" action="{{ route('meta.ad-accounts.update', $ad_account) }}" class="space-y-4">
            @csrf @method('PUT')

            @include('meta.ad_accounts._form')
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('meta.ad-accounts.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">
                    Cancelar
                </a>
            </div>
        </form>
