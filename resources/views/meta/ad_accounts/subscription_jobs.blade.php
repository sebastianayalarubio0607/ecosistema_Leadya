...
@section('title', 'Jobs Suscripcion Meta Ad Accounts')
@section('subtitle', 'Cola y fallos de suscripcion de cuentas publicitarias')

@section('header_actions')
    <form method="POST" action="{{ route('meta.ad-accounts.subscription-jobs.scan') }}">
        @csrf
        <button class="px-4 py-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-white border border-white/10">Revisar cuentas</button>
    </form>
    ...
    <div class="grid gap-4">
        <section class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
            <div class="flex items-center justify-between gap-3">
                ...
                <form method="POST" action="{{ route('meta.ad-accounts.subscription-jobs.queued.release-all') }}">
                    @csrf
                    ...
                    <tbody class="divide-y divide-white/10 text-white/80">
                        @forelse($queuedJobs as $job)
                            <tr class="hover:bg-white/5">
                                ...
                                <td class="px-3 py-2">
                                    <form method="POST" action="{{ route('meta.ad-accounts.subscription-jobs.queued.release', $job->id) }}">
                                        @csrf
        ...
        <section class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
            <div class="flex items-center justify-between gap-3">
                ...
                <form method="POST" action="{{ route('meta.ad-accounts.subscription-jobs.failed.retry-all') }}">
                    @csrf
                ...
                <table class="w-full min-w-[1150px] text-sm" data-sortable-table>
                    <thead class="bg-white/5 text-white/70">
                        <tr>
                            ...
                            <x-sort-header :index="2" label="Cuenta" />
                    ...
                    <tbody class="divide-y divide-white/10 text-white/80">
                        @forelse($failedJobs as $failed)
                            <tr class="hover:bg-white/5 align-top">
                                ...
                                <td class="px-3 py-2">
                                    <form method="POST" action="{{ route('meta.ad-accounts.subscription-jobs.failed.retry', $failed) }}">
                                        @csrf
