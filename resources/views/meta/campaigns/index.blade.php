...
@section('subtitle', 'Campañas (Clientes -> Account -> Campaign)')

@section('header_actions')
    <a href="{{ route('meta.campaigns.create') }}" class="px-4 py-2 rounded-xl bg-indigo-500/30 hover:bg-indigo-500/40 text-white border border-white/10">+ Nueva</a>
@endsection
    ...
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                <a href="{{ route('meta.campaigns.index') }}" class="w-full text-center px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Limpiar</a>
            </div>
        </form>
        ...
        <div class="w-full max-w-full overflow-x-auto rounded-xl border border-white/10 [scrollbar-gutter:stable]" data-sortable-table-wrap>
            ...
            <table class="w-full min-w-[950px] text-sm" data-sortable-table>
                ...
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($items as $it)
                        @php($customerNames = ($it->account?->customers ?? collect())->pluck('name')->implode(', '))
                        <tr class="hover:bg-white/5">
                            <td class="px-3 py-2">{{ $customerNames !== '' ? $customerNames : 'Sin asignar' }}</td>
                            <td class="px-3 py-2">
                                {{ $it->account?->name ?? '—' }}
                                <div class="text-xs text-white/50">{{ $it->account?->meta_account_id }}</div>
                            </td>
                            <td class="px-3 py-2">
                                {{ $it->name }}
                                <div class="text-xs text-white/50">{{ $it->meta_campaign_id }}</div>
                            </td>
                            <td class="px-3 py-2">{{ $it->objective }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-white/60">No hay campañas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
