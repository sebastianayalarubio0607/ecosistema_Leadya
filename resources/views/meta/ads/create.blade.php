...
@section('content')
    @php($ad = new \App\Models\MetaAd())

    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
        <form method="POST" action="{{ route('meta.ads.store') }}" class="space-y-4">
            @csrf
            @include('meta.ads._form')
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('meta.ads.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
