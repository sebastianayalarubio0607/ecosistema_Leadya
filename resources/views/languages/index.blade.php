...
@section('subtitle', 'Catálogo de idiomas para el generador de URLs')
                ...
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($languages as $language)
                        <tr class="hover:bg-white/5">
                            <td class="px-3 py-2">{{ $language->id }}</td>
                            <td class="px-3 py-2">
                                <span class="px-2 py-1 rounded-lg bg-white/10 border border-white/10 text-xs">{{ $language->code }}</span>
                            </td>
                            <td class="px-3 py-2">{{ $language->name }}</td>
                            <td class="px-3 py-2">
                                <span class="px-2 py-1 rounded-lg text-xs border {{ $language->is_active ? 'bg-emerald-500/10 border-emerald-300/20 text-emerald-200' : 'bg-white/10 border-white/10 text-white/70' }}">
                                    {{ $language->is_active ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                            <td class="px-3 py-2">{{ optional($language->updated_at)->format('Y-m-d H:i') }}</td>
                            <td class="px-3 py-2">
                                <div class="flex items-center gap-2">
                                    <a class="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 border border-white/10 text-xs"
                                       href="{{ route('languages.show', $language) }}">Ver</a>
                                    <a class="px-3 py-1.5 rounded-lg bg-indigo-500/20 hover:bg-indigo-500/30 border border-white/10 text-xs"
                                       href="{{ route('languages.edit', $language) }}">Editar</a>
                                    <form action="{{ route('languages.destroy', $language) }}" method="POST">
                                        ...
                                        <button class="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-300/20 text-xs"
                                                onclick="return confirm('¿Eliminar idioma?')">
                                            Eliminar
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-white/60">No hay idiomas registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
