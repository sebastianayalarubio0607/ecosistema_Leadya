...
@section('title', 'Google Ads Ads')
@section('subtitle', 'Métricas diarias de anuncios por cliente')
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                ...
                <div class="md:col-span-2">
                    <label class="block mb-1 text-white/70">Ad ID</label>
                    <input name="google_ad_id" value="{{ request('google_ad_id') }}" class="w-full rounded-xl border border-white/10 p-2 bg-slate-900/60 text-white" />
                </div>
                <div class="md:col-span-3">
                    <label class="block mb-1 text-white/70">Ad Status</label>
                    <input name="ad_status" value="{{ request('ad_status') }}" class="w-full rounded-xl border border-white/10 p-2 bg-slate-900/60 text-white" />
                </div>
                <div class="md:col-span-12 flex gap-2">
                    ...
                    <a href="{{ route('google-ads.ads.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Limpiar</a>
                </div>
            </form>
                ...
                <table class="w-full min-w-[1150px] text-sm" data-sortable-table>
                    <thead class="bg-white/5 text-white/70">
                        <tr>
                            ...
                            <x-sort-header :index="3" label="Ad Group" />
                            <x-sort-header :index="4" label="Ad" />
                            <x-sort-header :index="5" label="Status" />
                            <x-sort-header :index="6" label="Impresiones" />
                            <x-sort-header :index="7" label="Clicks" />
                            <x-sort-header :index="8" label="Conversiones" />
                            <x-sort-header :index="9" label="Costo" />
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/10 text-white/80">
                        @forelse($items as $item)
                            <tr class="hover:bg-white/5">
                                ...
                                <td class="px-3 py-2">{{ $item->ad_group_name ?: '—' }}</td>
                                <td class="px-3 py-2 font-mono text-xs">{{ $item->google_ad_id }}</td>
                                <td class="px-3 py-2">{{ $item->ad_status ?: '—' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-3 py-8 text-center text-white/60">No hay anuncios sincronizados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
