...
@section('title', 'Google Ads Campaigns')
@section('subtitle', 'Métricas diarias de campañas por cliente')
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-4 space-y-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                ...
                <div class="md:col-span-3">
                    <label class="block mb-1 text-white/70">Campaign Status</label>
                    <input name="campaign_status" value="{{ request('campaign_status') }}" class="w-full rounded-xl border border-white/10 p-2 bg-slate-900/60 text-white" />
                </div>
                <div class="md:col-span-12 flex gap-2">
                    ...
                    <a href="{{ route('google-ads.campaigns.index') }}" class="px-4 py-2 rounded-xl bg-zinc-950/25 hover:bg-white/10 text-white border border-white/10">Limpiar</a>
                </div>
            </form>
                ...
                <table class="w-full min-w-[950px] text-sm" data-sortable-table>
                    <thead class="bg-white/5 text-white/70">
                        <tr>
                            ...
                            <x-sort-header :index="4" label="Impresiones" />
                            <x-sort-header :index="5" label="Clicks" />
                            <x-sort-header :index="6" label="Conversiones" />
                            <x-sort-header :index="7" label="Costo" />
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/10 text-white/80">
                        @forelse($items as $item)
                            <tr class="hover:bg-white/5">
                                ...
                                <td class="px-3 py-2">
                                    {{ $item->campaign_name ?: '—' }}
                                    <div class="text-xs text-white/50">{{ $item->google_campaign_id }}</div>
                                </td>
                                <td class="px-3 py-2">{{ $item->campaign_status ?: '—' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 py-8 text-center text-white/60">No hay campañas sincronizadas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
