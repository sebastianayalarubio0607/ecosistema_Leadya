...
@section('title', $geo->name)
@section('subtitle', 'Detalle del geo')

@section('header_actions')
    <a href="{{ route('geos.edit', $geo) }}"
    ...
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-6 text-white/80">
        <div class="grid gap-4">
            <div>
                ...
                <div class="mt-1">{{ $geo->id }}</div>
            </div>
            <div>
                <div class="text-sm text-white/50">Código</div>
                <div class="mt-1">{{ $geo->code }}</div>
            </div>
            <div>
                <div class="text-sm text-white/50">Nombre</div>
                <div class="mt-1">{{ $geo->name }}</div>
            </div>
            <div>
                <div class="text-sm text-white/50">Estado</div>
                <div class="mt-1">{{ $geo->is_active ? 'Activo' : 'Inactivo' }}</div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <div class="text-sm text-white/50">Creado</div>
                    <div class="mt-1">{{ optional($geo->created_at)->format('Y-m-d H:i') }}</div>
                </div>
                <div>
                    <div class="text-sm text-white/50">Actualizado</div>
                    <div class="mt-1">{{ optional($geo->updated_at)->format('Y-m-d H:i') }}</div>
                </div>
            </div>
            <div class="pt-3">
                <form method="POST" action="{{ route('geos.destroy', $geo) }}" onsubmit="return confirm('¿Seguro que deseas eliminar este geo?');">
                    @csrf
                    @method('DELETE')
