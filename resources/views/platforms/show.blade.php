...
@section('subtitle', 'Detalle de la plataforma')
    ...
    <div class="rounded-2xl border border-white/10 bg-zinc-950/25 backdrop-blur p-6 text-white/80">
        <div class="grid gap-4">
            <div>
                ...
                <div class="mt-1">{{ $platform->id }}</div>
            </div>
            <div>
                <div class="text-sm text-white/50">Código</div>
                <div class="mt-1">{{ $platform->code }}</div>
            </div>
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="mt-1">{{ optional($platform->created_at)->format('Y-m-d H:i') }}</div>
                </div>
                <div>
                    <div class="text-sm text-white/50">Actualizado</div>
                    <div class="mt-1">{{ optional($platform->updated_at)->format('Y-m-d H:i') }}</div>
                </div>
            </div>
            <div class="pt-3">
                <form method="POST" action="{{ route('platforms.destroy', $platform) }}" onsubmit="return confirm('¿Seguro que deseas eliminar esta plataforma?');">
                    @csrf
                    @method('DELETE')
