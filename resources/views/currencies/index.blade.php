...
@section('subtitle', 'Catalogo de divisas para leads y conversiones')
                ...
                <tbody class="divide-y divide-white/10 text-white/80">
                    @forelse($currencies as $currency)
                        <tr class="hover:bg-white/5">
                            <td class="px-3 py-2">{{ $currency->id }}</td>
                            <td class="px-3 py-2">{{ $currency->name }}</td>
                            <td class="px-3 py-2 font-mono">{{ $currency->code }}</td>
                            <td class="px-3 py-2">
                                <span class="px-2 py-1 rounded-lg text-xs border {{ $currency->status ? 'bg-emerald-500/10 border-emerald-300/20 text-emerald-200' : 'bg-white/10 border-white/10 text-white/70' }}">
                                    {{ $currency->status ? 'Activa' : 'Inactiva' }}
                                </span>
                            </td>
                            <td class="px-3 py-2">{{ optional($currency->updated_at)->format('Y-m-d H:i') }}</td>
                            <td class="px-3 py-2">
                                <div class="flex items-center gap-2">
                                    <a class="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 border border-white/10 text-xs"
                                       href="{{ route('currencies.show', $currency) }}">Ver</a>
                                    <a class="px-3 py-1.5 rounded-lg bg-indigo-500/20 hover:bg-indigo-500/30 border border-white/10 text-xs"
                                       href="{{ route('currencies.edit', $currency) }}">Editar</a>
                                    <form action="{{ route('currencies.destroy', $currency) }}" method="POST">
                                        ...
                                        <button class="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-300/20 text-xs"
                                                onclick="return confirm('Eliminar divisa?')">
                                            Eliminar
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-white/60">No hay divisas registradas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
